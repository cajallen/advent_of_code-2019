...
def get_line_segments(instructions):
	...
	for instr in instructions:
		if instr[0] == "L":
			x2 = x1 - instr[1]
			segments.append([x1,y1,x2,y1,"h", instr[1]])
			x1 = x2
		elif instr[0] == "R":
			x2 = x1 + instr[1]
			segments.append([x1,y1,x2,y1,"h", instr[1]])
			...
			y2 = y1 - instr[1]
			segments.append([x1,y1,x1,y2,"v", instr[1]])
			y1 = y2
		elif instr[0] == "D":
			y2 = y1 + instr[1]
			segments.append([x1,y1,x1,y2,"v", instr[1]])
...
def get_overlap(seg1, seg2):
	...
	if (seg1[orient] == "v" and seg2[orient] == "h"):
		# if the vertical segment's x value is greater than the other segments min x value, but lesser than its max x value
		if (min(seg2[x1],seg2[x2]) <= seg1[x1] and seg1[x1] <= max(seg2[x1],seg2[x2])):
			if (min(seg1[y1],seg1[y2]) <= seg2[y1] and seg2[y1] <= max(seg1[y1],seg1[y2])):
				# return the vertical segments x value, and the horizontal segments y value
				return [seg1[x1],seg2[y1]]
	elif (seg1[orient] == "h" and seg2[orient] == "v"):
		# if the vertical segment's x value is greater than the other segments min x value, but lesser than its max x value
		if (min(seg1[x1],seg1[x2]) <= seg2[x1] and seg2[x1] <= max(seg1[x1],seg1[x2])):
			if (min(seg2[y1],seg2[y2]) <= seg1[y1] and seg1[y1] <= max(seg2[y1],seg2[y2])):
				# return the vertical segments x value, and the horizontal segments y value
...
overlaps = []
seg1_length = 0
for seg1 in get_line_segments(wire1_instructions):
	seg2_length = 0
	for seg2 in get_line_segments(wire2_instructions):
		overlap = get_overlap(seg1, seg2)
		if (overlap != False):
			overlaps.append(abs(overlap[0] - seg1[0]) + abs(overlap[1] - seg1[1]) + abs(overlap[0] - seg2[0]) + abs(overlap[1] - seg2[1]) + seg1_length + seg2_length)
